...
def test_explanation(query, clickedText):
    messages = [{"role": "system", "content": f'In 100 tokens or less explain why this lab test might be helpful in the below history/complaint. Do not include test name in response: {clickedText}'}]
    messages.append({"role": "user", "content":f'History/complain: {query}'})
    response = openai.ChatCompletion.create(model="gpt-4", messages=messages, temperature = 1, max_tokens = 100)
